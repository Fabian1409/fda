...
fn visualize(events: &[Event], hosts: &BTreeMap<String, usize>) -> Result<(), Box<dyn Error>> {
    ...
    for event in events.iter() {
        ...
        let label = event.title.chars().next().unwrap();
...
fn assign_vector_clocks(events: &mut Vec<Event>, hosts: Vec<String>) {
    ...
    for event in events {
        ...
        if event.title.eq("Receive event") {
            ...
            let vec = messages.get(&(sender.clone(), *send_clock)).unwrap();
            ...
            messages.insert(
                (host.clone(), *event.clock.get(&host).unwrap()),
                vclocks.get(&host).unwrap().clone(),
            );
        }

        event.v = vclocks.get(&host).unwrap().clone();
    }
}
...
fn count_concurrent_events(events: &[Event]) -> usize {
    ...
    for e1 in events {
        for e2 in events {
            if e1.v.iter().zip(e2.v.iter()).any(|(e1, e2)| e1 > e2)
                & e2.v.iter().zip(e1.v.iter()).any(|(e2, e1)| e2 > e1)
            {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let hosts = hosts.into_keys().collect();
